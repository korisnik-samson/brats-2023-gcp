...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.tau = nn.Parameter(
            torch.ones((num_heads, window_size[0] * window_size[1], window_size[0] * window_size[1]))
        )
...
class FinalPatchExpand_X4(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.expand(x)
        B, L, C = x.shape
        assert L == H * W, "input feature has the wrong size"
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        assert H == self.img_size[0] and W == self.img_size[1], \
            f"Input image size ({H} x {W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=4, num_classes=1000,
        ...
        print(
            f"** SwinTransformerSys Expand Initial **"
            f"-Depths: {depths}; Depths Decoder: {depths_decoder}; Drop Path Rate: {drop_path_rate}; Num Classes: {num_classes}-"
        )
    ...
    def up_x4(self, x):
        ...
        assert L == H * W, "input features has the wrong size"
...
if __name__=='__main__':
    ...
    print('*- --- Test Model SwinTransformerSys --- -*')
